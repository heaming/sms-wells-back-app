...
모든 고객이 머물면서 모든 것을 해결하는 '장소' **K-station**<br/>
**K-station**은 교원 그룹사(빨간펜, 구몬, 웰스, 라이프)의 고객이 모든 업무를 해결하는 ALL-IN-ONE PLACE로, 보다 더 빠른 서비스를 지원하기 위한 영업관리 플랫폼입니다.
분화되어 있던 시스템 및 DB를 하나로 통합하고, Biz/IT프로세스 개선과 설계로 시대의 변화와 고객 대응에 유연한 서비스를 제공합니다.

* 서비스/모바일 영역 개발
* 팀 내 맡은 역할 :
    - 백엔드 / 프론트엔드 영업관리 페이지(게시판) 개발
