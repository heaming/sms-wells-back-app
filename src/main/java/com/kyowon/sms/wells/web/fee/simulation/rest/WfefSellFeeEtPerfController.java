...
public class WfefSellFeeEtPerfController {
    ...
    @ApiImplicitParams(value = {
        ...
        @ApiImplicitParam(name = "inqrDv", value = "조회구분 1: 건수&실적, 2: 계정순증, 3: 판매상세, 4: 순증상세", defaultValue = "1", paramType = "query"),
    ...
    @GetMapping("/cts")
    public List<SearchRes> getSellFeeEtPerfs(
        @Valid
        SearchReq dto
    ) {
    ...
    @GetMapping("/details")
    public List<SearchDetailRes> getSellFeeEtPerfDetails(
        @Valid
        SearchReq dto
    ) {
    ...
    @GetMapping("/can-nincs")
    public List<SearchCanNincRes> getSellFeeEtPerfCanNincs(
        @Valid
        SearchReq dto
    ) {
        return service.getSellFeeEtPerfCanNincs(dto);
    }
}
