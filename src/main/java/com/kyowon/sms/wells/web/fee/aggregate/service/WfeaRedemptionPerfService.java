...
public class WfeaRedemptionPerfService {
    ...
    public void aggregateRedemptionOfFeePerformance(String baseYm, String ogTpCd, String perfAgrgCrtDvCd, String cntrPerfCrtDvCd) {
        ...
        BizAssert.isTrue(mapper.selectRedemptionConfirmStatus(baseYm, perfAgrgCrtDvCd) == 0, "MSG_ALT_CANT_AGRG_BY_CONFIRM", new String[] {baseYm, code.getCodeName()});

        /* 되물림상태코드 생성 */
        mapper.insertRedemptionStatus(baseYm, perfAgrgCrtDvCd);

        /* 기생성 데이터 삭제 */
        mapper.deleteCommonRedempPerfData("TB_FEAM_NTORP_PERF_MM_CL", baseYm, ogTpCd, perfAgrgCrtDvCd);
        mapper.deleteCommonRedempPerfData("TB_FEAM_NTORP_CNTR_MM_CL", baseYm, ogTpCd, perfAgrgCrtDvCd);
        ...
        /* 파트너 개인실적 합계 생성 */
        mapper.insertSumOfCntrDataToPrtnrForIndiv(baseYm, ogTpCd, perfAgrgCrtDvCd, perfAtcCds);

        /* 조직 합계 생성 */
        mapper.insertSumOfCntrDataToPrtnrForOg(baseYm, ogTpCd, perfAgrgCrtDvCd, perfAtcCds);
