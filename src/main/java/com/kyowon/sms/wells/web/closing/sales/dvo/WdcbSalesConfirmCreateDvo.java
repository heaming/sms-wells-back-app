package com.kyowon.sms.wells.web.closing.sales.dvo;

import lombok.Getter;
import lombok.Setter;
import lombok.ToString;

/**
 * <pre>
 * 매출확정생성 서비스 DVO
 * </pre>
 *
 * @author WOO SEUNGMIN
 * @since 2023-04-13
 */
@Getter
@Setter
@ToString
public class WdcbSalesConfirmCreateDvo {

    private String cntrNo;
    private int cntrSn;
    private String slRcogDt;
    private String kwGrpCoCd;
    private String bzHdqDvCd;
    private String ogTpCd;
    private String prtnrNo;
    private String pdHclsfId;
    private String pdMclsfId;
    private String pdLclsfId;
    private String pdCd;
    private String cstNo;
    private String copnDvCd;
    private String bzrno;
    private String sellTpCd;
    private String sellTpDtlCd;
    private String sellInflwChnlDtlCd;
    private int sellQty;
    private int sellAmt;
    private int sellAmtVat;
    private int sellSplAmt;
    private int cntrTam;
    private int subscAmt;
    private int rentalRgstCost;
    private int rentalAmt;
    private int rentalDscAmt;
    private int rentalPtrm;
    private int rentalTn;
    private int istmPcamAmt;
    private int istmFeeLvyAmt;
    private int istmAmt;
    private int istmMcn;
    private int mmIstmAmt;
    private int dscAmt;
    private int cntramDpAmt;
    private int bilDscAmt;
    private int ovrCtrDpAmt;
    private int prmTn;
    private int totPrmAmt;
    private int prmExpAmt;
    private String prmStrtY;
    private String prmStrtMm;
    private String prmEndY;
    private String prmEndMm;
    private int prmMcn;
    private int prmDscr;
    private int prmDscAmt;
    private int prmDpAmt;
    private int prmRfndAmt;
    private int prmRplcAmt;
    private int prmSlAmt;
    private int nomSlAmt;
    private int spmtSlAmt;
    private int nomDscAmt;
    private int spmtDscAmt;
    private int slCtrAmt;
    private int slCanAmt;
    private String slStpYn;
    private String cntrStlmFshDtm;
    private String cntrStrtdt;
    private String canDt;
    private int slDc;
    private int svAmt;
    private int nomIntAmt;
    private int mlgSlAmt;
    private String ostrDtm;
    private String sppDtm;
    private String istDtm;
    private String reqdDtm;
    private String svDt;
    private int pvdaOjPcam;
    private int pvdaAmt;
    private String slRcogClsfCd; /*매출인식분류코드*/
    private String slRcogDvCd; /*매출인식구분코드*/
    private String lgstItmGdCd; /*물류품목등급코드*/
    private int reimPcsvCs; /*변상택배비용*/
    private int pcsvReimAmt; /*택배변상금액*/
    private String iostDt;
    private int slQty;
    private String rtngdYn;
    private String frisuYn;
    private String fgptYn;
}
