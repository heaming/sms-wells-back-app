...
import com.kyowon.sms.wells.web.service.stock.dto.WsnaMdProductReturningGoodsMgtDto;
...
import com.kyowon.sms.wells.web.service.stock.mapper.WsnaMdProductReturningGoodsSaveMapper;
...
@Slf4j
public class WsnaMdProductReturningGoodsSaveService {

    private static final String RETURN_INSIDE = "262"; // 택배반품출고유형코드
    private final WsnaMdProductReturningGoodsSaveMapper saveMapper;
    private final WsnaMdProductReturningGoodsMgtMapper mapper;
    private final WsnaMdProductReturningGoodsMgtConverter converter;
    ...
    public int saveMdProductReturningGoods(List<SaveReq> dtos) {
        ...
        List<WsnaMdProductReturningGoodsSaveDvo> dvos = converter.mapSaveReqToMdProductReturningGoodsDvo(dtos);

        // 물류수불처리 DVO
        List<WsnaMdProductReturningGoodsSaveDvo> logisticsDvos = new ArrayList<WsnaMdProductReturningGoodsSaveDvo>();
            ...
            */
            if ("10".equals(dvo.getWkPrgsStatCd()) && !StringUtils.isEmpty(dvo.getRsgFshDt())) {
                ...
                */
                ...
                counselRes = counselService.saveWellsCounsel(setMdProductReturnGoodsWellsCounselReqIvoSaveReq(dvo));
            ...
            saveCount = saveMdProductReturningGoodsOstrs(logisticsDvos);
    ...
    @Transactional
    public int saveMdProductReturningGoodsOstrs(List<WsnaMdProductReturningGoodsSaveDvo> dvos) {
        ...
        for (WsnaMdProductReturningGoodsSaveDvo dvo : dvos) {
            ...
            String itmOstrNo = this.mapper.selectNextItmOstrNo(
                // 택배반품 임시사용 261 사용
                new WsnaMdProductReturningGoodsMgtDto.FindItmOstrNoReq(RETURN_INSIDE, ostrDt)
            );
            ...
            for (WsnaPcsvReturningGoodsSaveProductDvo productsVo : products) {
                ...
                List<WsnaMdProductReturningGoodsSaveDvo> logisticsVo = new ArrayList<WsnaMdProductReturningGoodsSaveDvo>();
                logisticsVo.add(dvo);
    ...
    private List<WsnaLogisticsInStorageAskReqDvo> setLogisticsInStorageAskReqDvo(
        List<WsnaMdProductReturningGoodsSaveDvo> logisticsVo
    ) {
        List<WsnaLogisticsInStorageAskReqDvo> AskReqList = new ArrayList<WsnaLogisticsInStorageAskReqDvo>();
        for (WsnaMdProductReturningGoodsSaveDvo dvo : logisticsVo) {
    ...
    private void selectExistSvpdCstSvWkRsIz(
        WsnaMdProductReturningGoodsSaveDvo dvo
    ) {
        int existCnt = saveMapper.selectExistSvpdCstSvWkRsIz(dvo);
        BizAssert.isTrue(existCnt == 0, "이미 완료 처리 되었습니다. 작업목록을 다시 확인 해주세요.");
    }

    private WellsCounselReqIvo setMdProductReturnGoodsWellsCounselReqIvoSaveReq(
        WsnaMdProductReturningGoodsSaveDvo dvo
    ) {
