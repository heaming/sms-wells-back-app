...
public class WsnaOutOfStorageByTaskController {
    ...
    @GetMapping("/paging")
    public PagingResult<HashMap<String, Object>> getOutOfStorageByTaskType(
        SearchReq dto,
        @Valid
        PageInfo pageInfo
    ) {
    ...
    public List<HashMap<String, Object>> getOutOfStorageByTaskType(
        SearchReq dto
    ) {
        return this.service.getOutOfStorageByTaskType(dto);
    }

    @ApiOperation(value = "업무유형별 자재출고현황 창고 컬럼구하기", notes = "업무유형별 / 서비스센터별로 자재출고집계현황을 엑셀다운로드 한다.")
    @GetMapping("/ware-fields")
    public List<WareField> getWareFields(
        SearchReq dto
    ) {
        return this.service.getWareFields(dto);
    }
}
