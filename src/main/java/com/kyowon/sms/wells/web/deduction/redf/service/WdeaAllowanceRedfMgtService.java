...
public class WdeaAllowanceRedfMgtService {
    ...
    public PagingResult<SearchRedfBizdTestRes> getRedfBizdMgt(SearchRedfBizdReq dto, PageInfo pageInfo) {
        ...
        if ("0202".equals(dto.redfAdsbTpCd())) {
            /* A~: 지구장 이하, B~: 지점장 이상
            *  ~1: 2019년 03월 이전
        ...
        if ("0203".equals(dto.redfAdsbTpCd())) {
            dvo = mapper.selectRedfBizdDelinquentMgts(dto, pageInfo); // M조직 연체
    ...
    public List<SearchRedfBizdTestRes> getRedfBizdMgtForExcelDownload(SearchRedfBizdReq dto) throws Exception {
        ...
        if ("0203".equals(dto.redfAdsbTpCd())) {
            dvo = mapper.selectRedfBizdDelinquentMgts(dto); // M조직 연체
